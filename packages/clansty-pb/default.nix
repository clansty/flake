...
writeScriptBin "pb"
  ''
    ...
    pb_post() {
      ...
      long_args="content:,expire:,name:,passwd:,file:,verbose,human,private,clip,dry"
      ...
      while [[ ''${1:0:1} == - ]]; do
        ...
        [[ $1 =~ ^-h|--human$ ]]    && { human="true"; shift 1; continue; };
    ...
    pb_update() {
      ...
      long_args="content:,expire:,passwd:,file:,verbose,human,clip,dry"
      TEMP=$(getopt -o "$short_args" --long "$long_args" -n "$script_name" -- "$@") \
        || return 1
      eval set -- "$TEMP";
